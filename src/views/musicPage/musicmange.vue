...
>>>>>>> 823c3f26bf6eb2058bfa147a7b214c5cdb1b5f23
  ...
  methods: {
    ...
    getUserMsg(id) {
      this.$axios
        .get(`/api/music/querySingleMusic/?id=` + id)
        .then(response => {
    ...
    async deleteOperator() {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        .then(() => {
          ...
          this.$axios
            .delete(
              `/api/music/deleteSingMusic/?id=` + this.multipleSelection[0].id
            )
            .then(response => {
              this.$message({
                type: 'success',
    ...
    confirm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          const data = this.$qs.stringify(this.music)
          this.listLoading = true
          this.dialogVisible = false
          if (this.dialogTitle === '增加') {
            this.$axios
              .post(`/api/music/addSingMusic`, data)
              .then(response => {
=======
            }).catch(() => {
              this.$message({
...
>>>>>>> 823c3f26bf6eb2058bfa147a7b214c5cdb1b5f23
            ...
            this.$axios
              .post(`/api/music/editSingMusic`, data)
              .then(response => {
...
<<<<<<< HEAD
    ...
    handleQueryByPage() {
      this.listLoading = true
      this.$axios
        .get('/api/music/queryAllMusic/?start=' + this.Query.start)
        .then(response => {
